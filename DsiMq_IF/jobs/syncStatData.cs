...
{
    public class syncStatData : IJob
    {
        ...
        public void Run()
        {
            ...
            try
            {
                                                                ...
                                                                Properties.Settings.Default.BadKey.ToString()))
                {
                    ...
                    int cc = dbconn.sqlConnection();
                    // 프로시저 수정(동봉된 프로시저 스키마 참조)★★★
                    dbconn.CmdText = UTIL.DBQuery.StaticsDataProc;
                    ...
                    isok = SpcCore.syncStatData(CustomerCode, ds.Tables[0]);
                    ...
                    if (isok)
                    {
                        ...
                        log.Debug("통계치데이터 " + ds.Tables[0].Rows.Count + "건 전송완료 IDXKEY : (" + idxkey + ")");
                        Msg("통계치데이터 " + ds.Tables[0].Rows.Count + "건 전송완료", 1);
                    }
                }
            }
            catch (Exception ie)
